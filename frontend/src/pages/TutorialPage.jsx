      ...
      <div className="max-w-4xl mx-auto px-6 space-y-8 relative z-10">
        ...
        <div className="bg-[#FF9B9B] dark:bg-rose-900/40 rounded-[32px] p-8 shadow-sm border border-rose-200 dark:border-rose-900/50 transition-colors">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            {[
              { title: "Quiz", subtitle: "2 Passport", active: true },
              { title: "Category 1", subtitle: "10 Kuis", active: false },
              { title: "Category 2", subtitle: "5 Kuis", active: false },
              { title: "Category 3", subtitle: "8 Kuis", active: false },
              { title: "Category 4", subtitle: "15 Kuis", active: false },
              { title: "Category 5", subtitle: "20 Kuis", active: false },
            ].slice(0, 4).map((item, idx) => (
              <div key={idx} className={`${item.active ? 'bg-[#93C5FD] dark:bg-blue-600' : 'bg-white/90 dark:bg-white/10'} rounded-2xl p-4 flex items-center justify-between hover:scale-[1.02] transition-transform cursor-pointer shadow-sm`}>
                <div>
                  <h4 className={`font-black text-[15px] ${item.active ? 'text-white' : 'text-slate-800 dark:text-white'} leading-tight`}>{item.title}</h4>
                  <p className={`text-[11px] font-bold ${item.active ? 'text-blue-100' : 'text-slate-500 dark:text-slate-300'}`}>{item.subtitle}</p>
                </div>
                <div className={`w-6 h-6 rounded-full flex items-center justify-center ${item.active ? 'bg-white/20 text-white' : 'bg-rose-100 dark:bg-rose-900/50 text-rose-400 dark:text-rose-300'}`}>
        ...
        <div className="bg-[#FF9B9B] dark:bg-rose-900/40 rounded-[32px] p-8 shadow-sm border border-rose-200 dark:border-rose-900/50 transition-colors">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              {[1, 2, 3].map((item) => (
                <div key={item} className="flex items-center gap-4 bg-white/90 dark:bg-white/10 rounded-2xl p-3 hover:shadow-md transition-shadow cursor-pointer">
                  <div className="w-16 h-16 bg-slate-200 dark:bg-black/30 rounded-xl overflow-hidden shrink-0">
                    <img src={`https://ui-avatars.com/api/?name=Tips+${item}&background=cbd5e1&color=475569`} alt="Thumbnail" className="w-full h-full object-cover opacity-80" />
                  </div>
                  <div>
                    <h4 className="font-black text-[13px] text-slate-800 dark:text-white leading-tight mb-1">Tips & Trick<br/>Wawancara</h4>
                    <p className="text-[10px] font-bold text-slate-500 dark:text-slate-300">2 Category</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-4">
              {[
                { title: "Tips & Trick Wawancara", time: "2 min ago" },
                { title: "Tutorial Bikin Wawancara...", time: "2 min ago" },
                { title: "Tips & Trick Wawancara", time: "1 hour ago" }
              ].map((video, idx) => (
                <div key={idx} className="flex items-center gap-4 bg-[#4A3B3B] dark:bg-zinc-900 rounded-2xl p-3 hover:shadow-md transition-shadow cursor-pointer border border-[#5A4B4B] dark:border-zinc-800">
                  ...
                  <div>
                    <h4 className="font-bold text-[13px] text-white leading-tight mb-1">{video.title}</h4>
                    <p className="text-[10px] font-medium text-slate-400">{video.time}</p>
                  </div>
                </div>
              ))}
            </div>
